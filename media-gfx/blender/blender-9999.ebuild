...
src_compile() {
	...
	cd "${WORKDIR}"/install/2.55/plugins/ \
...
#	fi
	...
	doexe "${WORKDIR}"/install/2.55/plugins/texture/*.so
	exeinto /usr/share/${PN}/${SLOT}/sequences
	doexe "${WORKDIR}"/install/2.55/plugins/sequence/*.so
	insinto /usr/include/${PN}/${SLOT}
	doins "${WORKDIR}"/install/2.55/plugins/include/*.h
	rm -r "${WORKDIR}"/install/2.55/plugins || die
